...
print(f"Loaded {len(AFRICAN_CROPS)} African crops into database")
...
print("Part 3 (ML Models & Weather) ready")
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        default_index = pages.index(st.session_state.get('current_page', "🏠 Home"))
        ...
        st.markdown(f"**Crop Types:** `{len(AFRICAN_CROPS)}` African crops")
        st.markdown(f"**Database:** `SQLite v3.0`")
...
print("Part 4 (UI Components) ready — NameError fixed by defining functions before page blocks")
...
if page == "🏠 Home":
    ...
    Incorporating world-class medical AI innovations from imaging to cardiac auscultation.
    </p>
    """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if 'medic_tab' in st.session_state:
        # We can't programmatically select tabs in Streamlit, but we can show a hint
